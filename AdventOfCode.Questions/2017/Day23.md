...
The code it's running seems to be a variant of the kind you saw recently on that [tablet](Day18.md). The general functionality seems _very similar_, but some of the instructions are different:
...
The _debug mode switch_ is wired directly to register `a`. You flip the switch _(From 'magic' to 'more magic'.)_, which makes _register `a` now start at `1`_ when the program is executed.

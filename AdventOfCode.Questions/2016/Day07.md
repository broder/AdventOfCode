...
An IP supports TLS if it has an Autonomous Bridge Bypass Annotation, or _ABBA_ _(Any similarity to the pattern it describes is purely coincidental.)_. An ABBA is any four-character sequence which consists of a pair of two different characters followed by the reverse of that pair, such as `xyyx` or `abba`. However, the IP also must not have an ABBA within any hypernet sequences, which are contained by _square brackets_.
